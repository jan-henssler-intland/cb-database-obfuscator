...
BEGIN
            ...
            SET details = REPLACE(REPLACE(REPLACE(REPLACE(details, CONCAT('{"id":', v_id, ',"name":"', v_name, '"}'),
                                  ...
                                  CONCAT('"user-', v_id, '\'s Personal Wiki"')) WHERE audit_trail_logs.id BETWEEN start_id AND max_id;
END //
...
DROP PROCEDURE IF EXISTS obfuscate_object_revision_batch;
            ...
            BEGIN
                ...
                THEN
                            ...
                            UPDATE task_type
                            SET prefix = SUBSTR(prefix, 1, 2) WHERE id BETWEEN start_id AND max_id;
                            COMMIT;
                END IF;
            END //
            ...
            BEGIN
                INSERT INTO prepare_statements VALUE('TRUNCATE TABLE object_revision_blobs;', 'Unfinished', 0, 0);
                INSERT INTO prepare_statements VALUE('UPDATE users
    SET name               = CONCAT(''user-'', id),
        ...
        unused2            = NULL,
        referrer_url       = NULL
    WHERE name NOT IN (''system'', ''computed.update'', ''deployment.executor'', ''scm.executor'');', 'Unfinished', 0, 0);
                INSERT INTO prepare_statements VALUE('TRUNCATE TABLE users_small_photo_blobs;', 'Unfinished', 0, 0);
                INSERT INTO prepare_statements VALUE('TRUNCATE TABLE users_large_photo_blobs;', 'Unfinished', 0, 0);

                INSERT INTO prepare_statements VALUE('DELETE FROM user_pref WHERE pref_id IN (63, 67, 2001, 2002);', 'Unfinished', 0, 0);
                INSERT INTO prepare_statements VALUE('TRUNCATE TABLE user_key;', 'Unfinished', 0, 0);
                INSERT INTO prepare_statements VALUE('UPDATE existing SET name = CONCAT(''Project'', proj_id), key_name = CONCAT(''K-'', proj_id) WHERE name <> ''codeBeamer Review Project'';', 'Unfinished', 0, 0);
                INSERT INTO prepare_statements VALUE('TRUNCATE TABLE object_job_schedule;', 'Unfinished', 0, 0);
                INSERT INTO prepare_statements VALUE('TRUNCATE TABLE object_quartz_schedule;', 'Unfinished', 0, 0);
                INSERT INTO prepare_statements VALUE('UPDATE label SET name = CONCAT(''LABEL'', id) WHERE name NOT IN (''FINISHED_TESTRUN_GENERATION'');', 'Unfinished', 0, 0);

                INSERT INTO prepare_statements VALUE('UPDATE workingset SET name = CONCAT(''WS-'', id), description = NULL WHERE name != ''member'';', 'Unfinished', 0, 0);
                INSERT INTO prepare_statements VALUE('SET FOREIGN_KEY_CHECKS = 0;
...
SET FOREIGN_KEY_CHECKS = 1;', 'Unfinished', 0, 0);
                ...
                INSERT INTO prepare_statements VALUE('TRUNCATE TABLE background_job_meta;', 'Unfinished', 0, 0);
                INSERT INTO prepare_statements VALUE('TRUNCATE TABLE background_step_result;', 'Unfinished', 0, 0);

                INSERT INTO prepare_statements VALUE('TRUNCATE TABLE background_step_context;', 'Unfinished', 0, 0);
                INSERT INTO prepare_statements VALUE('TRUNCATE TABLE QRTZ_BLOB_TRIGGERS;', 'Unfinished', 0, 0);
                INSERT INTO prepare_statements VALUE('TRUNCATE TABLE QRTZ_CALENDARS;', 'Unfinished', 0, 0);
                INSERT INTO prepare_statements VALUE('TRUNCATE TABLE QRTZ_CRON_TRIGGERS;', 'Unfinished', 0, 0);
                INSERT INTO prepare_statements VALUE('TRUNCATE TABLE QRTZ_FIRED_TRIGGERS;', 'Unfinished', 0, 0);
                INSERT INTO prepare_statements VALUE('TRUNCATE TABLE QRTZ_LOCKS;', 'Unfinished', 0, 0);
                INSERT INTO prepare_statements VALUE('TRUNCATE TABLE QRTZ_PAUSED_TRIGGER_GRPS;', 'Unfinished', 0, 0);
                INSERT INTO prepare_statements VALUE('TRUNCATE TABLE QRTZ_SCHEDULER_STATE;', 'Unfinished', 0, 0);
                INSERT INTO prepare_statements VALUE('TRUNCATE TABLE QRTZ_SIMPLE_TRIGGERS;', 'Unfinished', 0, 0);
                INSERT INTO prepare_statements VALUE('TRUNCATE TABLE QRTZ_SIMPROP_TRIGGERS;', 'Unfinished', 0, 0);
            END //
DELIMITER ;